...
export default function BeforeAfterPage() {
  ...
  const cmsCases: DisplayCase[] = (dbCases || []).map(item => ({ id: item.id, title: item.title || 'Patient transformation', description: item.description || '', before: item.before_image, after: item.after_image, beforeAlt: item.before_alt || `Before ${item.title || 'dental treatment'}`, afterAlt: item.after_alt || `After ${item.title || 'dental treatment'}` }));
  const staticCases: DisplayCase[] = clinicalCases.map(item => ({ id: item.id, title: item.title, description: item.description, before: item.before_image, after: item.after_image, beforeAlt: item.before_alt, afterAlt: item.after_alt }));
